...
保存佬朋友列表、打捞规则、请求统计、设置、通知凭据、云存档绑定状态、刷新状态和更新检查缓存。
...
用于云存档绑定、状态检查、备份和恢复。备份上传可迁移配置，包括佬朋友、打捞规则、请求统计、设置，以及已配置的 Telegram Bot Token / Chat ID。
...
- 用户活动或网站内容：用户可见的 linux.do 公开活动摘要、关注列表、帖子/回复/反应线索，用于好友视图和佬有料。
...
>
> 数据优先保存在浏览器扩展本地存储中。你可以按需启用云存档，把可迁移配置备份到 linuxdo-cloud-save.lafish.workers.dev；也可以配置 Telegram Bot Token / Chat ID，在有新佬有料时发送 Telegram 通知。扩展还会检查 GitHub Release 以提示新版本。
>
